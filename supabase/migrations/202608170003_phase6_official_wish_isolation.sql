-- Non-destructive production hotfix.
-- Stage 1 diagnostics have a NULL idempotency_key and remain untouched. This
-- migration changes only RPC scope; it does not delete or rewrite any rows.
...
begin
  ...
  select count(*) into v_active from public.wishes
    where owner_id=v_owner and idempotency_key is not null and status in ('sealed','expired');
...
begin
  ...
  select count(*), jsonb_build_object('wishCount',count(*),'sealedCount',count(*) filter(where status='sealed'),'expiredCount',count(*) filter(where status='expired'),'abandonedCount',count(*) filter(where status='abandoned'),'purchaseIntentCount',count(*) filter(where status='purchased_intent'),'abandonedListedAmount',coalesce(sum(counted_amount) filter(where status='abandoned'),0)) into v_total, v_summary
    from public.wishes where owner_id=v_owner and idempotency_key is not null;
  select coalesce(jsonb_agg(to_jsonb(w)), '[]'::jsonb) into v_items from (
    select * from public.wishes where owner_id=v_owner and idempotency_key is not null
    order by case when status='expired' then 0 when status='sealed' then 1 else 2 end, expires_at asc, updated_at desc
    offset v_offset limit v_page_limit
  ) w;
...
begin
  ...
  delete from public.wishes where id=p_wish_id and owner_id=auth.uid() and idempotency_key is not null;
  return found;
end; $$;
...
begin
  ...
  delete from public.wishes
    where owner_id=auth.uid() and idempotency_key is not null and status in ('purchased_intent','abandoned');
