...
import { ProductSearchService, normalizeClientQuery } from './products-service.js';
...
const productService = fixtureMode ? null : new ProductSearchService({ url: import.meta.env.VITE_SUPABASE_URL, publishableKey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY });
...
const productSummary = (product) => {
  const real = product.provider === 'taobao';
  const image = product.imageUrl ? `<img src="${product.imageUrl}" alt="${escape(product.title)}" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'image-fallback',textContent:'图片加载失败'}))" />` : '<div class="image-fallback">图片暂缺</div>';
  const price = real ? `<p>销售价 ${displayPrice(product.price)} · <b>${product.priceLabel} ${displayPrice(product.finalPrice)}</b></p>` : `<p>标价 ${displayPrice(product.listPrice)} · 销售价 ${displayPrice(product.sellingPrice)} · <b>预估到手价 ${displayPrice(product.estimatedPrice)}</b></p>`;
  return `<div class="summary">${image}<div><strong>${escape(product.title)}</strong>${price}</div></div>`;
};

function shell(content) {
  root.innerHTML = `<main class="app-shell"><header><a class="brand" href="#home" data-action="home">看山</a><span class="mode-badge">${fixtureMode ? '阶段 3 · 开发测试数据' : '阶段 3 · 淘宝联盟实时候选'}</span><button class="quiet-button" data-action="wishes">我的愿望</button></header>${content}</main>`;
...
function render() {
  clearInterval(timer);
  if (!fixtureMode && !productService.configured) { root.innerHTML = `<main class="app-shell unavailable"><h1>商品服务尚未配置</h1><p>当前页面不会回退到开发 fixture。请配置公开 Supabase URL 与 publishable key 后再进行真实商品搜索；阶段 1 诊断仍在 <a href="diagnostic.html">诊断页</a>。</p></main>`; return; }
...
function renderProducts() { shell(`<section><p class="eyebrow">${fixtureMode ? '开发测试数据' : '淘宝联盟实时候选'} · ${escape(flow.query)}</p><h1>从候选中选择一件</h1><p>实际价格以淘宝结算页为准。</p><div class="products">${flow.products.map((product) => `<article class="product-card">${productSummary(product)}<p class="source">来源：${fixtureMode ? '开发 fixture（非淘宝实时结果）' : '淘宝联盟实时候选'} · ${fixtureMode ? '' : '刚刚获取'}</p>${product.bundlePrice && product.bundlePriceCondition ? `<p>凑单参考价 ${displayPrice(product.bundlePrice)}：${escape(product.bundlePriceCondition)}</p>` : ''}<button class="secondary" data-product="${product.itemId}">选择这件</button></article>`).join('')}</div><button class="quiet-button" data-action="home">重新输入</button></section>`); root.querySelectorAll('[data-product]').forEach((button) => button.addEventListener('click', () => { const product = flow.products.find((item) => item.itemId === button.dataset.product); if (!product.promotionUrl) { flow = { ...flow, error: '该候选缺少有效官方推广链接，不能继续。' }; save(States.ERROR); return; } save(fixtureMode ? States.EVIDENCE_LOADING : States.PRODUCT_SELECTED, { product }); })); }

function renderProductSelected() { shell(`<section><p class="eyebrow">阶段 3 边界</p><h1>商品已选定；知乎证据将在阶段 4 接入。</h1>${productSummary(flow.product)}<p class="notice">当前仅完成淘宝联盟真实候选选择，不展示 fixture 知乎证据、不启动倒计时，也不打开推广链接。</p><button class="secondary" data-action="products">返回重选商品</button><button class="quiet-button" data-action="home">开始新的搜索</button></section>`); }
function renderProductEmpty() { shell(`<section class="center-state"><p class="eyebrow">淘宝联盟实时候选</p><h1>暂未找到可展示的候选</h1><p>本次返回结果为空，或全部因字段不完整被过滤；没有使用 fixture 顶替。</p><button class="primary" data-action="home">换一个关键词</button></section>`); }
