...
const sessionMissing = (error) => error?.name === 'AuthSessionMissingError' || error?.code === 'session_not_found' || /auth session missing|session missing/i.test(error?.message ?? '');
...
export class AuthService {
  ...
  async ensureSession() {
    ...
    if (!this.initializing) this.initializing = (async () => {
      ...
      // First visit has no persisted session. Supabase exposes this as
      // AuthSessionMissingError; it is not a network or permission failure.
      if (error && !sessionMissing(error)) throw new AuthServiceError('authentication_required');
      const result = await this.client.auth.signInAnonymously();
      if (result.error || !result.data?.user) throw new AuthServiceError('authentication_required');
  ...
  async signOut({ scope = 'local' } = {}) { if (this.client) { const { error } = await this.client.auth.signOut({ scope }); if (error) throw new AuthServiceError('request_failed'); } }
}
