...
export const wishMessages = Object.freeze({ invalid_duration: '请选择 24、48 或 72 小时。', invalid_product: '商品快照不完整，不能创建愿望。', active_limit_reached: '同时保管中的愿望最多为 5 条。', duplicate: '该愿望已创建，无需重复提交。', wish_not_expired: '愿望尚未到期，暂不能作决定。', not_found: '未找到该愿望。', migration_rejected: '匿名愿望迁移未获授权。', request_failed: '愿望服务请求失败，请稍后重试。' });
...
// PostgREST can expose a stable function exception in different fields. Keep
// the mapping narrow and never surface the raw provider/database text.
export function extractWishErrorCode(error) {
  if (!error || typeof error !== 'object') return 'request_failed';
  for (const field of ['message', 'code', 'details', 'hint']) {
    const candidate = error[field];
...
export class WishesService {
  ...
  async rpc(name, args) { await this.auth.ensureSession(); const { data, error } = await this.client.rpc(name, args); if (error) throw new WishesServiceError(extractWishErrorCode(error)); return data; }
  ...
  async list({ offset = 0, limit = 20 } = {}) { const result = await this.rpc('list_my_custody_wishes', { p_offset: offset, p_limit: Math.min(20, Math.max(1, limit)) }); return result?.items ? result : { items: Array.isArray(result) ? result : [], nextOffset: null, hasMore: false, summary: null }; }
  ...
  async deleteAccount() { await this.auth.ensureSession(); const { data, error } = await this.client.functions.invoke('delete-my-account', { body: {} }); if (error || !data?.ok) throw new WishesServiceError('request_failed'); return true; }
}
