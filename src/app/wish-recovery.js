 ...
 */
export function recoveryPlan({ trigger, currentView, currentState, currentRecordId, wishes }) {
  if (trigger === RecoveryTriggers.INITIAL_LOAD) {
    const target = recoveryTarget(wishes);
    return target.reason === 'none_active'
      ? { action: 'keep_current', ...target }
      : { action: 'replace_view', ...target };
  }
