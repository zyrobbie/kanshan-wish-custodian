...
- `npm test`：49/49 通过。
- `npm run check`：通过（含密钥扫描、49 项单元测试、阶段 1–5 静态基线、生产构建和构建产物扫描）。
